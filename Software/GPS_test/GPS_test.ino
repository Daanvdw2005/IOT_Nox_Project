...
void loop() {
  // Controleer of er GPS-data beschikbaar is
  if (GPSSerial.available() > 0) {
    Serial.println("GPS-data ontvangen, aan het parsen...");
    while (GPSSerial.available() > 0) {
      char c = GPSSerial.read();
      Serial.print(c);  // Print ruwe NMEA-data voor debugging
      if (gps.encode(c)) {  // Parseer NMEA-data
        if (gps.location.isValid()) {  // Controleer of er een geldige fix is
          ...
          Serial.println("Geen geldige GPS-fix...");
        }
      }
    }
  } else {
  ...
  // Print huidige coördinaten (ook als geen fix)
